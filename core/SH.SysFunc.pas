unit SH.SysFunc;
 ...
 const
  ...
  // reserved space for internal implemented system functions
  ...
  SYS_STR_Rep          = 36; // [ ] replace substrig
 ...
 type
  ...
  TExternalFile = record
   ...
   ShortName : string;         // dll short name (only file_name)
 ...
 var
  ...
  SysNames  : TDictionary<AnsiString,Int64> = Nil;
  SysFuncs  : TArray<TSystemFunction> = Nil;
...
// ************************************************************************** //
 ...
 function TExternalFile.LoadModule(const FN:string; out Error:string):Boolean;
  begin
   ...
   ShortName := ReplaceStr(ShortName,'.DLL',''); // remove ".dll" extension
   Error     := '';
   Exit(true);
  end;
 ...
 function TExternalFile.LoadSymbol(const FN:string; out Error:string):Boolean;
  ...
  begin
   //
   nam := DecorateName(FN);
   if SysNames.ContainsKey(nam) then
    begin
     Error := Format('External symbol "%s" already exists',[nam]);
     Exit(true); // actually is not error
    end;
   //
   if Module = 0 then
    begin
     Error := Format('Can not load "%s" due module handle not initialized',[FN]);
     Exit(false);
    end;
   //
...
// ************************************************************************** //
 ...
 function TExternals.ModuleIndex(const FN:string):Int32;
  ...
  begin
   ...
   for i:=0 to High(Items) do
    if Items[i].ShortName = FN then Exit(i);
  end;
 ...
 function TExternals.ImportSymbol(const ModuleName,SymbolName:string; out Error:string):Boolean;
  var
   ...
   e : TExternalFile;
  begin
   i := ModuleIndex(Uppercase(ModuleName));
   if i = -1 then
    begin
   ...
   for i:=0 to High(Items) do
    begin
     ...
     for j:=0 to High(Items[i].Items) do
      SysNames.Remove(Items[i].Items[j]);
     // free library
     Items[i].FreeModule;
    end;
   Items := Nil;
   // restore to base system functions set
...
// ************************************************************************** //
 ...
 function FunctionIndex(const F:PFrame):Int32;
  ...
  begin with F^ do begin
   Result := -1;
   if RT.FN.TryGetValue(SP0.StrPtr,idx)
    then SP0.i8 := idx
    else SP0.i8 := -1;
  end; end;
...
initialization
 ...
 // CONSOLE I/O
 SysNames.Add('%SYS.CLRSCR'  ,SYS_CONSOLE_Clear   ); SysFuncs[SYS_CONSOLE_Clear   ] := Nil;
 SysNames.Add('%SYS.CLRLIN'  ,SYS_CONSOLE_ClrLine ); SysFuncs[SYS_CONSOLE_ClrLine ] := Nil;
 SysNames.Add('%SYS.GETPOS'  ,SYS_CONSOLE_GetPos  ); SysFuncs[SYS_CONSOLE_GetPos  ] := Nil;
 SysNames.Add('%SYS.SETPOS'  ,SYS_CONSOLE_SetPos  ); SysFuncs[SYS_CONSOLE_SetPos  ] := Nil;
 SysNames.Add('%SYS.PRINT'   ,SYS_CONSOLE_Print   ); SysFuncs[SYS_CONSOLE_Print   ] := CONSOLE_Print;
 SysNames.Add('%SYS.PRINTL'  ,SYS_CONSOLE_PrintL  ); SysFuncs[SYS_CONSOLE_PrintL  ] := CONSOLE_PrintLine;
 SysNames.Add('%SYS.INPUT'   ,SYS_CONSOLE_Input   ); SysFuncs[SYS_CONSOLE_Input   ] := CONSOLE_Input;
 SysNames.Add('%SYS.TXTATTR' ,SYS_CONSOLE_TextAttr); SysFuncs[SYS_CONSOLE_TextAttr] := Nil;

 // FILE I/O
 SysNames.Add('%SYS.EXISTS'  ,SYS_FILE_Exists     ); SysFuncs[SYS_FILE_Exists     ] := Nil;
 SysNames.Add('%SYS.FOPEN'   ,SYS_FILE_Open       ); SysFuncs[SYS_FILE_Open       ] := Nil;
 SysNames.Add('%SYS.FCLOSE'  ,SYS_FILE_Close      ); SysFuncs[SYS_FILE_Close      ] := Nil;
 SysNames.Add('%SYS.RDBUFFER',SYS_FILE_ReadBuf    ); SysFuncs[SYS_FILE_ReadBuf    ] := Nil;
 SysNames.Add('%SYS.WRBUFFER',SYS_FILE_WriteBuf   ); SysFuncs[SYS_FILE_WriteBuf   ] := Nil;
 SysNames.Add('%SYS.RDLINE'  ,SYS_FILE_ReadLn     ); SysFuncs[SYS_FILE_ReadLn     ] := Nil;
 SysNames.Add('%SYS.WRLINE'  ,SYS_FILE_WriteLn    ); SysFuncs[SYS_FILE_WriteLn    ] := Nil;
 SysNames.Add('%SYS.EOF'     ,SYS_FILE_Eof        ); SysFuncs[SYS_FILE_Eof        ] := Nil;
